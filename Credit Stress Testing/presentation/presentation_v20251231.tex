...
\title{Credit Stress Testing and Vasicek Model}
...
% ==========================================================
\begin{frame}{Credit Stress Testing}
\justifying
\small

The \textbf{financial crisis of 2008} led to more rigorous regulation of the financial sector. One key component of the new regulatory framework was the \textbf{introduction of stress testing}.

\medskip

The purpose of a stress test is to demonstrate that a \textbf{financial institution can withstand a severe - but plausible - economic downturn}. Such a downturn is represented through a \textbf{detailed economic scenario} that typically specifies the projected evolution of real GDP growth, the unemployment rate, inflation, the house price index, stock indices, government bond yields, foreign exchange rates, and interest rates over the next three years.

\medskip

Every large financial institution is expected to have the \textbf{ability and capacity to conduct stress tests on both a regular and an ad-hoc basis}.

\medskip

Stress testing plays a crucial role in \textbf{maintaining financial stability}, particularly amid evolving economic uncertainties and systemic risks.

\end{frame}

% ==========================================================
\begin{frame}{Credit Stress Testing \textit{(cont. 1)}}
\justifying
\small

In the Eurozone, large financial institutions are required to conduct the following \textbf{types of stress tests}:
\begin{enumerate}
\item \textbf{ICAAP (Internal Capital Adequacy Assessment Process)} - an annual internal stress test whose methodology, macroeconomic scenarios, and results are reviewed by the European Central Bank (ECB).
\item \textbf{EU-wide EBA stress test} - a biannual stress test coordinated by ECB following a standardized methodology and macroeconomic scenarios issued by the European Banking Authority (EBA) to ensure comparability across institutions.
\item \textbf{Ad-hoc stress test} - triggered by ECB in response to a specific adverse event (e.g., the U.S. - China trade conflict or the Russia - Ukraine war).
\end{enumerate}

\end{frame}

% ==========================================================
\begin{frame}{Credit Stress Testing \textit{(cont. 2)}}
\justifying
\small

Stress testing constists of \textbf{several modules} covering banking book, trading book, operational risk, liqudity risk, and \textbf{credit risk}. The following text is focused on credit stress testing.

\medskip

\textbf{Credit stress testing} consists of \textbf{two main components}:
\begin{enumerate}
\item \textbf{Macroeconomic model} - linear regression model that translates a macroeconomic scenario into a value for a systematic factor. This factor reflects the phase of the economic cycle (recession vs. expansion).
\item \textbf{Vasicek model} - uses systematic factor estimated through macroeconomic model to stress Probability of Default (PD). The stressed probability of default is used to calculate various credit risk metrics such as Expected Loss (EL), Risk-weighted Assets (RWA), Expected Credit Loss (ECL), which is lifetime expected credit loss used for accounting purposes.
\end{enumerate}
...
\item \textbf{Loss Given Default} (LGD) - the proportion of the exposure that is lost in the event of default, expressed as a percentage of EAD.
\item \textbf{Expected Loss} (EL) - the average loss incurred on a given loan or credit facility under normal conditions.
\end{itemize}
...
The Vasicek model is a \textbf{cornerstone of modern credit risk analysis and regulation}. Originally introduced by Oldrich Vasicek in 1987, the model provides a simple yet powerful framework for quantifying portfolio credit risk by linking individual default probabilities to a common systematic factor.

\medskip

Its analytical tractability and intuitive structure make it particularly suitable for assessing the impact of macroeconomic shocks on portfolio-level losses. The model assumes that \textbf{defaults are driven by latent variables} representing the underlying credit quality of borrowers, which are correlated through their exposure to the \textbf{systematic factor}. This dependence structure allows the Vasicek model to capture the key feature of credit portfolios - \textbf{default clustering} during economic downturns - while remaining computationally efficient.
...
Equation (1) for EL assumes a \textbf{long-term average}, the so-called \textbf{Through-the-Cycle (TTC)} Probability of Default (PD). As its name suggests, the TTC PD represents an average probability of default observed throughout the entire economic cycle. In practice, this value can be derived from a counterparty's external credit rating (for large corporates) or from internal scoring models (for retail exposures).

\medskip

The Vasicek model introduces the concept of adjusting the TTC PD to reflect the \textbf{macroeconomic environment} described through macroeconomic scenario. This stressed probability of default, conditional on the prevailing macroeconomic conditions, is referred to as the \textbf{Point-in-Time (PIT)} Probability of Default. Unlike the TTC PD, which is constant, the PIT PD varies with the macroeconomic environment - it is higher during recessions and lower during expansions.
...
\begin{itemize}
    ...
    \item $a, b > 0$ are \textbf{loading coefficients}.
\end{itemize}
...
The Vasicek model further assumes that $\mathrm{Var}[x_{i,t}] = 1$, which implies the following restriction
\begin{equation*}
...
Assuming independence between $Z_t$, $\varepsilon_{i, t}$, and $\varepsilon_{j, t}$, it can be proved that their correlation is equal to $\rho$. This explains why $\rho$ is often referred to as the \textbf{correlation parameter of the Vasicek model}.
{\tiny
\begin{align*}
...
% ==========================================================
\begin{frame}{Interpretation of Systematic Factor}
\justifying
\small
...
Intuitively, there is a \textbf{relation between value of systematic factor and defaults}. Significantly negative values of the factor, which indicates economic recession, should corresponds with increased number of defaults and vice versa.

\end{frame}


% ==========================================================
\begin{frame}{Observed Default Rates and Historical Realizations of Systematic Factor}
\justifying
\small

Observed default rates\footnote{\tiny{Observed default rates represent the proportion of a portfolio that defaults within a given time period. Unlike the probability of default, which is a theoretical (ex-ante) concept, the observed default rate is an empirical (ex-post) measure. For example, if a portfolio has 1,000 obligors at the beginning of the year and 10 default by year-end, the observed default rate for that year is 1\%.}} for a given portfolio can be used to infer historical realizations of the systematic factor $Z_t$, which are a crutial input to macroecnomic model calibration.
...
% ==========================================================
\begin{frame}{Historical Realizations of Systematic Factor and Requirements of Vasicek Model}
\justifying
\small
...
% ==========================================================
\begin{frame}{Historical Realizations of Systematic Factor and Requirements of Vasicek Model \textit{(cont. 1)}}
\justifying
\small
...
% ==========================================================
\begin{frame}{Historical Realizations of Systematic Factor and Requirements of Vasicek Model \textit{(cont. 2)}}
\justifying
\small
...
\quad \text{and} \quad 
PD_i^{\mathrm{TTC}} = \Phi \Big( \sqrt{1 - \rho} \mathrm{E}[\Phi^{-1}(PD_{i,t})] \Big),
\end{equation}
...
% ==========================================================
\begin{frame}{Historical Realizations of Systematic Factor and Normality Assumption}
\justifying
\small
...
% ==========================================================
\begin{frame}{Calibration Process of Macroeconomic Model}
\justifying
\small
...
% ==========================================================
\begin{frame}{Pitfalls of Calibration Process}
\justifying
\small
...
\begin{itemize}
    \tiny{
    \item Over-representation of ``good'' or ``bad'' economic periods may lead to a biased model as the latest economic cycle is not always fully covered.
    \item Inevitably short calibration periods (15 - 20 non-overlapping annual data points) can cause re-calibration instability.
    }
\end{itemize}
...
% ==========================================================
\begin{frame}{Pitfalls of Calibration Process \textit{(cont. 1)}}
\justifying
\small
